looker.plugins.visualizations.add({
  id: "custom-single-cross-filter-for-ec",
  ...
  options: {
    ...
    list_container_radius: {
      label: "List Box Radius",
      type: "string",
      display_size: 'third',
      default: "4px",
      placeholder: "4px",
    ...
    element.innerHTML = `
      <style>
        ...
        .cv-container {
          ...
          /* Inherit to respect Looker Theme Font */
          font-family: inherit;
        ...
        .scroll-area {
          ...
          border-radius: var(--list-container-radius);
          padding: 2px;
          min-height: 0;
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    setVar('--list-container-radius', fixPx(config.list_container_radius, '4px'));
    setVar('--row-radius', fixPx(config.row_border_radius, '3px'));
    setVar('--measure-radius', fixPx(config.measure_border_radius, '2px'));
    ...
    try {
      ...
      // ★★★ 追加: Null値の表示設定取得 (デフォルトtrue) ★★★
      const showNulls = config.show_null_values !== false;
      ...
      data.forEach(row => {
        ...
        // ★★★ 修正: Null値の判定とフィルタリング ★★★
        if (dimCell) {
          const isNull = (dimCell.value === null || dimCell.value === undefined);

          // Nullかつ表示設定がOFFの場合はスキップ
          if (isNull && !showNulls) return;

          const val = String(dimCell.value);
          // Lookerが返すrenderedが空の場合は補完する
          let rendered = dimCell.rendered;
          if (rendered === undefined || rendered === null) {
            rendered = isNull ? "(Null)" : `${val}`;
          }
      ...
      // 外部で変数を宣言（applyFilter等からアクセスできるようにする）
      ...
      const applyFilter = () => {
        const term = (enableSearch && searchInput) ? searchInput.value : "";
